...
from source.gemgem_1 import find_matching_gems
...
def run_game():
    ...
    while True:
        ...
        if clicked_space and not first_selected_gem:
            ...
            game_board[first_swapping_gem["x"]][first_swapping_gem["y"]] = (
                second_swapping_gem["imageNum"]
            )
            game_board[second_swapping_gem["x"]][second_swapping_gem["y"]] = (
                first_swapping_gem["imageNum"]
            )

            # See if this is a matching move.
            matched_gems = get_switched_list(
                find_matching_gems(get_switched_mat(game_board))
            )
...
def draw_moving_gem(gem, progress):
    ...
    r = pygame.Rect(
        (pixel_x + move_x, pixel_y + move_y, GEM_IMAGE_SIZE, GEM_IMAGE_SIZE)
    )
            ...
            if board_copy[x][y] == EMPTY_SPACE:
                ...
                for offset_x, offset_y in ((0, -1), (1, 0), (0, 1), (-1, 0)):
                    ...
                    neighbor_gem = get_gem_at(
                        get_switched_mat(board_copy), y + offset_y, x + offset_x
                    )
        ...
        for y in range(BOARD_HEIGHT - 2, -1, -1):
            if board_copy[x][y + 1] == EMPTY_SPACE and board_copy[x][y] != EMPTY_SPACE:
                ...
                dropping_gems.append(
                    {"imageNum": board_copy[x][y], "x": x, "y": y, "direction": DOWN}
                )
...
def animate_moving_gems(board, gems, points_text, score):
    ...
    while progress < 100:  # animation loop
        ...
        FPSCLOCK.tick(FPS)
        progress += (
            MOVE_RATE  # progress the animation a little bit more for the next frame
        )
